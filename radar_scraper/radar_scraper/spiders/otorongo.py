...
class OtorongoSpider(scrapy.Spider):
    ...
    start_urls = ["https://otorongo.club/2026/sentencias/"]
    ...
    def parse(self, response):
        ...
        for page_link in response.css('div.text-center ul.pagination li a::attr(href)').getall():
            yield response.follow(url=page_link,
                                  callback=self.parse,
                                  meta={'year': year}
                                  )
    ...
    def parse_candidate(self, response):
        ...
        name = response.css('h2.text-white::text').get()
        ...
        jobs = response.css('div.content > div')[3].css('li')
